...
def consistentAStar (max_height, current_state, goal) :
    #parse the state and goal to a separate matrix
    current_state_matrix = common.saveMatrix(current)
    ...
    while priority_queue:
        ...
        if common.equal(current_state, goal_matrix):
            ...
            if current_state not in visited:
                child = common.childrenWithHeuristic(cost, current_state, movements, path, max_height, goal_matrix)
...
if __name__ == "__main__":
    ...
    maxHeight = int(lines[0])
    current = lines[1]
    goal = lines[2]

    #Call A* method
    consistentAStar(maxHeight, current, goal)
